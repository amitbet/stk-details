...
const { fetchSctrJson } = require("../shared/sctrService");
...
function createApp({ rendererDir, enableCors } = {}) {
  ...
  app.post("/api/parse-csv", upload.single("file"), async (req, res) => {
    try {
      ...
      const parsed = parseCsvForTickers(text);
  ...
  app.post("/api/fetch-sctr", async (req, res) => {
    try {
      ...
      if (normalized.length === 0) return res.json({ records: [] });
...
function parseCsvForTickers(csvText) {
